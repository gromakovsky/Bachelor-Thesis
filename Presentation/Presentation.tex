...
много вершин в графе, и склеиваются, получается полигон, описывающий воду.
\item Полигон смещается внутрь, потому что, во-первых, корабли не плавают
...
осуществляется на плоскости. Кратчайшая траектория на сфере
не является отрезком, поэтому не будем допускать слишком
длинные рёбра, тогда расхождение будет пренебрежимо мало.
\item Также добавим рёбра, получившиеся в результате схлопывания отрезков
при смещении полигона, и рёбра через 180-ый меридиан.
\end{itemize}
}
