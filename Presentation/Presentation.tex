...
\begin{frame}{Описание задачи}
    ...
    \only<3> {
        \begin{figure}
            \includegraphics[width=.88\textwidth]{Introduction/my-good}
...
Первая метрика берёт для каждой вершины первого пути длину кратчайшего
расстояния до второго пути, и её значение — максимум этих расстояний.
...
\item Поскольку граф не является графом видимости, то кратчайший путь
  в нём не обязан быть действительно кратчайшим путём между двумя
  точками. Кратчайший путь в графе не обязан быть действительно
  кратчайшим маршрутом между двумя точками, обычно его можно
  сократить. Дальше по тексту.
...
вершин первого пути. Из фиктивной вершины заупстим обход Дейкстры,
...
\newcommand{\potentialsupdatepicture}[1] {
    \includegraphics[width=\textwidth, clip=true, trim = 280pt 0 20pt 0]{Solution/potentials-update/#1}
}
